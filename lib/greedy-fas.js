...
const DEFAULT_WEIGHT_FN = __.constant(1)
...
function greedyFAS (g, weightFn) {
  ...
  // Expand multi-edges
  return __.flatten(__.map(results, function (e) {
...
function removeNode (g, buckets, zeroIdx, entry, collectPredecessors) {
  ...
  __.each(g.inEdges(entry.v), function (edge) {
  ...
  __.each(g.outEdges(entry.v), function (edge) {
    const weight = g.edge(edge)
    const w = edge.w
...
function buildState (g, weightFn) {
  ...
  const buckets = __.range(maxOut + maxIn + 3).map(function () { return new List() })
  const zeroIdx = maxIn + 1

  __.each(fasGraph.nodes(), function (v) {
