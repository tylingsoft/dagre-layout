...
function findSmallestWidthAlignment (g, xss) {
  return _.min(xss, function (xs) {
    const min = _.min(xs, function (x, v) { return x - width(g, v) / 2 })
    const max = _.max(xs, function (x, v) { return x + width(g, v) / 2 })
    return max - min
  })
}
...
function alignCoordinates (xss, alignTo) {
  const alignToMin = _.min(alignTo)
  const alignToMax = _.max(alignTo)
  ...
  _.each(['u', 'd'], function (vert) {
    _.each(['l', 'r'], function (horiz) {
      ...
      const delta = horiz === 'l' ? alignToMin - _.min(xs) : alignToMax - _.max(xs)
      if (delta) {
...
function balance (xss, align) {
  return _.mapValues(xss.ul, function (ignore, v) {
    if (align) {
      ...
      const xs = _.sortBy(_.pluck(xss, v))
      return (xs[1] + xs[2]) / 2
    }
  })
}
