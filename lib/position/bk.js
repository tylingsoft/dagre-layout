...
function alignCoordinates (xss, alignTo) {
  const alignToMin = _.min(_.values(alignTo)) || 0 // { c: NaN, a: NaN, b: NaN, d: NaN }
  const alignToMax = _.max(_.values(alignTo)) || 0
  ...
  _.each(['u', 'd'], function (vert) {
    _.each(['l', 'r'], function (horiz) {
      ...
      const delta = horiz === 'l' ? alignToMin - (_.min(_.values(xs)) || 0) : alignToMax - (_.max(_.values(xs)) || 0)
      if (delta) {
