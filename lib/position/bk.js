...
function alignCoordinates (xss, alignTo) {
  const alignToMin = _.min(_.values(alignTo))
  const alignToMax = _.max(_.values(alignTo))
  ...
  _.each(['u', 'd'], function (vert) {
    _.each(['l', 'r'], function (horiz) {
      ...
      const delta = horiz === 'l' ? alignToMin - _.min(_.values(xs)) : alignToMax - _.max(_.values(xs))
      if (delta) {
