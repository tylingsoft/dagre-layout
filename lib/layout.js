...
function updateInputGraph (inputGraph, layoutGraph) {
  __.each(inputGraph.nodes(), function (v) {
  ...
  __.each(inputGraph.edges(), function (e) {
    const inputLabel = inputGraph.edge(e)
    const layoutLabel = layoutGraph.edge(e)

    inputLabel.points = layoutLabel.points
    if (__.has(layoutLabel, 'x')) {
      inputLabel.x = layoutLabel.x
      inputLabel.y = layoutLabel.y
    }
  })
...
function buildLayoutGraph (inputGraph) {
  ...
  g.setGraph(__.merge({},
    ...
    __.pick(graph, graphAttrs)))

  __.each(inputGraph.nodes(), function (v) {
    const node = canonicalize(inputGraph.node(v))
    g.setNode(v, __.defaults(selectNumberAttrs(node, nodeNumAttrs), nodeDefaults))
    g.setParent(v, inputGraph.parent(v))
  })

  __.each(inputGraph.edges(), function (e) {
    const edge = canonicalize(inputGraph.edge(e))
    g.setEdge(e, __.merge({},
      edgeDefaults,
      selectNumberAttrs(edge, edgeNumAttrs),
      __.pick(edge, edgeAttrs)))
  })
...
function assignRankMinMax (g) {
  ...
  __.each(g.nodes(), function (v) {
    ...
    if (node.borderTop) {
      ...
      maxRank = __.max(maxRank, node.maxRank)
    }
  })
...
function translateGraph (g) {
  ...
  __.each(g.nodes(), function (v) { getExtremes(g.node(v)) })
  __.each(g.edges(), function (e) {
    const edge = g.edge(e)
    if (__.has(edge, 'x')) {
      getExtremes(edge)
  ...
  __.each(g.edges(), function (e) {
    const edge = g.edge(e)
    __.each(edge.points, function (p) {
      p.x -= minX
      p.y -= minY
    })
    if (__.has(edge, 'x')) { edge.x -= minX }
    if (__.has(edge, 'y')) { edge.y -= minY }
  })
...
function removeBorderNodes (g) {
  __.each(g.nodes(), function (v) {
    if (g.children(v).length) {
      ...
      const l = g.node(__.last(node.borderLeft))
      const r = g.node(__.last(node.borderRight))
...
function insertSelfEdges (g) {
  ...
  __.each(layers, function (layer) {
    let orderShift = 0
    __.each(layer, function (v, i) {
      const node = g.node(v)
      node.order = i + orderShift
      __.each(node.selfEdges, function (selfEdge) {
...
function selectNumberAttrs (obj, attrs) {
  return __.mapValues(__.pick(obj, attrs), Number)
}

function canonicalize (attrs) {
  const newAttrs = {}
  __.each(attrs, function (v, k) {
